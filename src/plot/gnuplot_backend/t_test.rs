...
pub(crate) fn t_test(
    ...
    figure
        ...
        .set(Title(format!("{}: Welch t test", gnuplot_escape(id.as_title()))))
        ...
        .plot(FilledCurve { x: &*xs, y1: &*ys, y2: zero }, |c| {
            c.set(DARK_BLUE).set(Label("t distribution")).set(Opacity(0.25))
        })
        .plot(Lines { x: &[t, t], y: &[0, 1] }, |c| {
